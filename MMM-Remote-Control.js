...
Module.register("MMM-Remote-Control", {
      ...
      let modules =
        payload.module === "all"
          ...
          : MM.getModules().filter((m) => {
              return (
                m.identifier === payload.module || m.name === payload.module
              );
            });
  ...
  getDom: function () {
    ...
    const displayqr =
      "<div><img src='./modules/MMM-Remote-Control/QRcodes/QR1.png' alt='qr-code' /><p>" +
      this.addresses[0] +
      "</p></div>";
    // wrapper.innerHTML =
    //   "http://" + this.addresses[0] + portToShow + "/remote.html";
    // wrapper.innerHTML =
    //   "<img src='./modules/MMM-Remote-Control/QRcodes/QR1.png' alt='qr-code' />";
    wrapper.innerHTML = displayqr;
