  ...
  Object.assign(
    {
      ...
      start: function () {
        ...
        fs.readFile(path.resolve(__dirname + "/remote.html"), function (
          err,
          data
        ) {
        ...
        fs.readFile(path.resolve(__dirname + "/modules.json"), (err, data) => {
          ...
          for (let i = 0; i < defaultModules.length; i++) {
            self.modulesAvailable.push({
              ...
              url:
                "https://github.com/MichMich/MagicMirror/wiki/MagicMirror%C2%B2-Modules#default-modules"
            });
            var module =
      ...
      monitorControl: function (action, opts, res) {
        ...
        let monitorStatusCommand =
          ...
          "monitorStatusCommand" in this.thisConfig.customCommand
            ...
            : "tvservice --status";
        if (
          ["MONITORTOGGLE", "MONITORSTATUS", "MONITORON"].indexOf(action) !== -1
        ) {
          screenStatus = exec(
            ...
            (error, stdout, stderr) => {
              if (
                stdout.indexOf("TV is off") !== -1 ||
                stdout.indexOf("false") !== -1
              ) {
      ...
      loadDefaultSettings: function () {
        ...
        fs.readFile(path.resolve(__dirname + "/settings.json"), function (
          err,
          data
        ) {
          if (err) {
        ...
        for (var k in interfaces) {
          for (var k2 in interfaces[k]) {
            ...
            if (address.family === "IPv4" && !address.internal) {
              ...
              QRCode.toFile(
                "./modules/MMM-Remote-Control/QRcodes/QR" + i + ".png",
                address.address,
                (err) => {
                  if (err) throw err;
                  console.log("done");
